# Licensing — the Protean Kit distribution

This repository is the distribution home of the Protean Kit. It is a
STANDALONE product built on the Hermes engine. It is not a git-fork of Hermes:
no upstream source or history is copied into this tree.

- **Source of truth:** `https://github.com/aska-digital/protean-kit`.
- The zone model below rests on this document and the README, not on a
  git-history boundary. Read this before contributing.

## Zones

The zone model is the one in the table below. Where a repository ships its own
committed `LICENSE` file, that file is authoritative for that repository.
...
| Composer layer | `kit.lock.json`, `install.sh`, `build/check-lock.py`, `build/check-ingredient-contract.py`, `choreography/`, `templates/personas/`, `registry/kit.yaml`, `AUDIT/` | **Apache-2.0** | The composition manifest, the installer, the composition gates, the identity archetypes, the orchestration contract, governance, and build evidence. |
| Ingredient layer | not in this repository | **MIT, per ingredient** | The ingredients the manifest pins are separate repositories. Each one commits its own `LICENSE` file, which is authoritative for it. This repository holds no ingredient payload: composition is manifest-only. |
...
| Product brand | `README.md`, `LICENSE` | Apache-2.0 (README), Apache-2.0 (LICENSE) | The product name is the distribution's brand. |

## Composition and pinning (lock version 1)

The distribution pins each ingredient by annotated tag plus peeled commit SHA
and a tree hash (`protean-tree-v1`). A pin is immutable: a moved tag is an
integrity failure, never a silent upgrade, and the only legal change to a lock is
a new tag or a `lockVersion` bump. The installer verifies the pin, installs into
a staging directory, promotes an ingredient only after its own gates pass, and
then re-hashes every written file against the cached blob. Ingredient licenses
are declared per ingredient and enforced by the ingredient-contract gate.
...
- This repo: the Protean Kit distribution (`aska-digital/protean-kit`) — Apache-2.0 composer layer + proprietary packs by contract; pinned ingredients are MIT under their own committed LICENSE files
